...
namespace rapidcsv
{
  ...
  public:
    ...
    /**
     * @brief    Get Column by Reference by Index
     */

    template<typename T>
     void GetColumnByRef(const size_t pColumnIdx, std::vector<T> &column) const
     {
       const ssize_t columnIdx = pColumnIdx + (mLabelParams.mRowNameIdx + 1);
       Converter<T> converter(mConverterParams);
       for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow)
    ...
    template<typename T>
      void GetColumnByRef(const std::string& pColumnName, std::vector<T> &column) const
      {
        ...
        GetColumnByRef<T>(columnIdx, column);
      }
  ...
  private:
    void ReadCsv()
    {
      ...
      stream.seekg(0, std::ios::beg);ReadCsv
    ...
    void ReadCsv(std::istream& pStream)
    {
           ...
           (mLabelParams.mColumnNameIdx + 1)))
      {
        ...
        {
          mRowNames[dataRow[mLabelParams.mRowNameIdx]] = k++;
        }
      }
    }
